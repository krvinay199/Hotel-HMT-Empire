/**
 * rooms.js — Hotel HMT Empire
 * Renders room cards from Firestore with fallback data.
 * Manages filter pills and GSAP horizontal scroll (desktop).
 */
...
// Fallback room data used when Firestore is not yet configured
const FALLBACK_ROOMS = [
  ...
  },
  {
    ...
    available:   true, // enabled for preview; will load from Firestore dynamically later
  },
];
...
/** Fetch rooms from Firestore; fall back to local data if Firestore fails */
...
/** Render room cards into the track */
function renderRoomCards(rooms) {
  ...
  // Force ScrollTrigger and Lenis to re-evaluate page height
...
function buildRoomCard(room) {
  ...
  // Book button → open booking modal pre-filled
...
/** Netflix-style carousel navigation controls */
function initCarouselArrows() {
  ...
  const updateArrowVisibility = () => {
    ...
    // Show/hide prev arrow based on scroll boundaries
    prevBtn.style.opacity = scrollLeft > 10 ? '1' : '0';
    prevBtn.style.pointerEvents = scrollLeft > 10 ? 'auto' : 'none';

    // Show/hide next arrow based on scroll boundaries
  ...
  const getScrollAmount = () => {
    ...
    return card ? card.offsetWidth + 24 : 400; // card width + flex gap
  };
  ...
  // Bind visibility updater to scroll actions (native snapping completes smoothly)
  wrapper.addEventListener('scroll', updateArrowVisibility, { passive: true });

  // Update initial arrow states when layout is calculated
