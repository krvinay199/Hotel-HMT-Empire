/**
 * rfq.js — Hotel HMT Empire
 * Multi-step Request for Quote form with validation, Firestore save, and EmailJS.
 */
...
/** Render step progress dots + connectors */
...
/** Build all 5 step panels */
function renderSteps() {
  const container = document.getElementById('rfq-steps-container');
  if (!container) return;

  container.appendChild(buildStep1()); // Event Type
  container.appendChild(buildStep2()); // Date Range
  container.appendChild(buildStep3()); // Guest Count
  container.appendChild(buildStep4()); // Requirements
  container.appendChild(buildStep5()); // Contact Info
...
function showStep(step) {
  ...
  // Scroll directly to the form card container smoothly on step change
  if (step > 1) {
    scrollTo('.rfq__form-container', -90);
  }

  // Programmatically focus contact input field on the final step
  if (step === 5) {
    setTimeout(() => {
